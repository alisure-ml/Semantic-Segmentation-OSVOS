...
class Dataset:
    ...
    def __init__(self, train_list, test_list, database_root, store_memory=True, data_aug=False):
            ...
            if store_memory:
                ...
                if data_aug:  # 数据增强
                    ...
                    for scale in data_aug_scales:
                        ...
                        # 左右翻转
                        if data_aug_flip:
                            self.images_train.append(np.array(img_sc.transpose(Image.FLIP_LEFT_RIGHT), dtype=np.uint8))
                            self.labels_train.append(np.array(label_sc.transpose(Image.FLIP_LEFT_RIGHT), dtype=np.uint8))
                        pass
                    pass
                else:  # 没有数据增强
                    if idx == 0:
        ...
        for idx, line in enumerate(test_paths):
            if self.store_memory:
                self.images_test.append(np.array(Image.open(os.path.join(database_root,
                                                                         str(line.split()[0]))), dtype=np.uint8))
                if (idx + 1) % 500 == 0:
    ...
    def next_batch(self, batch_size, phase):
        ...
        if phase == 'train':
            ...
            raise Exception("error in next_batch")
        pass
