...
def load_vgg_imagenet(ckpt_path):
    ...
    """
    var_to_shape_map = tf.train.NewCheckpointReader(ckpt_path).get_variable_to_shape_map()
    vars_corresp = dict()
    for v in var_to_shape_map:
        if "conv" in v:
            vars_corresp[v] = slim.get_model_variables(v.replace("vgg_16", "osvos"))[0]
        pass
    return slim.assign_from_checkpoint_fn(ckpt_path, vars_corresp)
...
def test(dataset, checkpoint_file, result_path, config=None):
    ...
    with tf.Session(config=config) as sess:
        ...
        for frame in range(0, dataset.get_test_size()):
            ...
            curr_frame = os.path.basename(curr_img[0]).split('.')[0] + '.png'
            ...
            print("{} Saving {}".format(datetime.now(), os.path.join(result_path, curr_frame)))
        pass
    pass
